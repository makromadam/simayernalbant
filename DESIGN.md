...
- **Double-bezel containers (`.bezel` + `.bezel-core`).** "Machined-hardware" nesting: an outer hairline shell (`1px rgba(255,150,86,.14)`, `2rem` radius, `6px` pad) wrapping an inner core with a concentric `calc(2rem - 6px)` radius and an `inset 0 1px 1px rgba(255,255,255,.14)` top highlight. Used for the App/Web video frames (`.bezel-core-dark` keeps a near-black core so `mix-blend-mode: screen` stays clean); the hero video is deliberately frameless (see below), and the About section is now a frameless scroll tunnel (see below). This is the new structural expression of the Glow-Not-Shadow Rule.
...
- **About — 3D typography tunnel.** The second section is a scroll-scrubbed vanishing-point experience: a `340vh` (`300vh` on mobile) scroll area with a `100vh` sticky viewport (`perspective: 1000px`, stage `transform-style: preserve-3d`). Three centred moments — the title (`#ff7200`, clamp display) then the two paragraphs (soft-white, `max-width 900px`) — each fly from `translateZ(-760px)` through a fully-readable centre (`translateZ(0)`, opacity plateau, no distortion) to `translateZ(+520px)` and out, driven by a vanilla `useScroll`/`useTransform`-equivalent (section scroll progress → per-moment Z/Y/opacity, rAF-throttled). No cards, no boxed/split layout — everything centred. `prefers-reduced-motion` collapses it to a calm static readable stack. No new libraries; consistent with the site's existing scroll-driven JS.
